...
export default function Home() {
  ...
  useEffect(() => {
    fetchSettings().then(s => {
      if (s) {
        setSettings(s);
        setSelectedProvider(s.activeProvider || "zhipu");
        // Set default model for selected provider
        if (s.providers[s.activeProvider || "zhipu"]) {
          setSelectedModel(s.providers[s.activeProvider || "zhipu"].models[0] || "");
        }
      }
    }).catch(e => {
  ...
  useEffect(() => {
    if (settings && settings.providers[selectedProvider]) {
  ...
  return (
    <div className="flex h-[calc(100vh-57px)] overflow-hidden">
      ...
      <div className="flex-1 flex flex-col bg-stone-950 overflow-hidden">
        ...
        {settings && (
          <div className="px-4 py-2 bg-stone-900/40 border-t border-stone-800/50 shrink-0">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <button
                  onClick={() => setShowProviderSelector(!showProviderSelector)}
                  className="flex items-center gap-2 px-3 py-1 bg-stone-800 hover:bg-stone-700 rounded-lg text-xs text-stone-300 transition-colors"
                >
                  <span className="text-sm">
                    {settings.providers[selectedProvider]?.type === "openai" && "🤖"}
                    {settings.providers[selectedProvider]?.type === "anthropic" && "🧠"}
                    {settings.providers[selectedProvider]?.type === "zhipu" && "🐱"}
                    {settings.providers[selectedProvider]?.type === "local" && "🏠"}
                  </span>
                  <span>{settings.providers[selectedProvider]?.name || "Select Provider"}</span>
                  <span className="text-stone-500">▼</span>
                </button>
                
                {selectedModel && (
                  <div className="flex items-center gap-2 px-3 py-1 bg-stone-800 rounded-lg">
                    <span className="text-xs text-stone-400">Model:</span>
                    <span className="text-xs text-stone-300">{selectedModel}</span>
                  </div>
                )}
              </div>
              
              {showProviderSelector && (
                <div className="absolute bottom-full left-4 mb-2 bg-stone-800 border border-stone-600 rounded-lg shadow-lg p-2 z-10">
                  ...
                  {selectedProvider && settings.providers[selectedProvider] && (
                    <div className="mt-2 pt-2 border-t border-stone-600">
                      <div className="text-xs text-stone-400 mb-1">Models:</div>
